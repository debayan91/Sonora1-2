...
// FIX 1: Modify the hook to accept a dependency array.
const useScrollAnimation = (deps = []) => {
    ...
    }, deps); // Use the passed-in dependencies.
    return (el) => {
...
const StickyBottomBar = ({ product, addItem, show, isDarkMode }) => {
...
const Prod = () => {
    ...
    // FIX 2: Pass the theme state to the hook to trigger it on change.
    const animateRef = useScrollAnimation([isDarkMode]);
      ...
      `}</style>
            <div className="font-roboto tracking-widest flex flex-col items-center relative">
                ...
                <main className="relative z-10 w-full">
                    <div className="flex flex-col justify-center items-center p-4 md:p-8 lg:p-12">
                        <section ref={animateRef} className={`${blurClass} ${bgClass} animate-on-scroll w-full max-w-screen-xl rounded-lg flex flex-col lg:flex-row p-4 md:p-8`}>
                            ...
                            <div className="lg:hidden w-full flex-shrink-0 order-2 lg:order-1 mt-8">
                                <div className="flex space-x-2 overflow-x-auto p-2">
                                    {productImages.map((img, index) => (
                                        <button key={index} onClick={() => setMainImage(img)} className={`flex-shrink-0 w-20 h-20 rounded-md cursor-pointer transition-opacity ${mainImage === img ? 'ring-2 ring-blue-500' : ''}`}>
                                            <img src={img} loading="lazy" className="h-full w-full object-cover rounded-md" alt={`${product.pname} view ${index + 1}`} />
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="hidden lg:flex h-[70vh] overflow-y-auto w-[10%] rounded-lg flex-col gap-4 p-2 order-1 no-scrollbar">
                                {productImages.map((img, index) => (
                                    <button key={index} onClick={() => setMainImage(img)} className={`w-full aspect-square rounded-lg cursor-pointer opacity-90 hover:opacity-100 transition-opacity ${mainImage === img ? 'ring-2 ring-blue-500' : ''}`}>
                                        <img src={img} loading="lazy" className="h-full w-full object-cover rounded-lg" alt={`${product.pname} view ${index + 1}`} />
                                    </button>
                                ))}
                            </div>

                            {/* Main Image & Zoom */}
                            <div className="w-full lg:w-[45%] flex items-center justify-center p-2 lg:p-8 relative order-1 lg:order-2">
                                <div onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)} className="relative w-full aspect-square">
                                    <img src={mainImage} onClick={() => handleImageClick(mainImage)} className="h-full w-full object-contain rounded-lg cursor-pointer" alt="Main product view" />
                                    {isHovering && (
                                        <div className={`hidden lg:block absolute left-full top-0 ml-4 w-full h-full border rounded-lg overflow-hidden shadow-lg z-50 ${isDarkMode ? 'bg-black border-gray-700' : 'bg-white border-gray-300'}`}>
                                            <div className="w-full h-full bg-cover bg-no-repeat rounded-lg" style={{ backgroundImage: `url(${mainImage})`, backgroundSize: '250%', backgroundPosition: `${zoomPosition.x}% ${zoomPosition.y}%` }}/>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <section ref={animateRef} className={`${blurClass} ${bgClass} animate-on-scroll w-full max-w-screen-xl rounded-lg flex flex-col p-4 md:p-8 mt-16 lg:mt-8 gap-8`}>
                    ...
                    <StickyBottomBar product={product} addItem={handleAddToCart} show={showStickyBar} isDarkMode={isDarkMode}/>
                </main>
            </div>
            <Footer />
        </>
