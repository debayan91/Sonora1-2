...
const Products = () => {
    ...
    return (
        <div className={`min-h-screen flex flex-col items-center relative ${bgColor}`}>
            ...
            <main className="relative z-10 w-full max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-16"> {/* Adjusted max-width and padding */}
                <div className={`p-4 sm:p-5 lg:p-6 mb-10 flex flex-col sm:flex-row justify-between items-center ${cardBg}`}> {/* Adjusted padding and flex direction */}
                    <h2 className={`text-xl font-roboto mb-4 sm:mb-0 ${textColor}`}>{sortedProducts.length} Products</h2> {/* Added margin for mobile */}
                    <div className="flex items-center">
                        ...
                        <select
                            ...
                            className={`border border-gray-950 px-2 py-1 text-sm focus:outline-none ${selectClasses}`}
                        >
                ...
                {sortedProducts.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8 lg:gap-10"> {/* Improved grid responsiveness and increased gap */}
                        {sortedProducts.map(product => (
                            <div
                                key={product.id}
                                onClick={() => clicky(product)}
                                className={` cursor-pointer overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 group ${cardBg} hover:scale-105`} // Refined shadows
                            >
                                <div className={`relative h-60 sm:h-72 lg:h-80 flex items-center justify-center overflow-hidden rounded-lg ${cardImageBg}`}> {/* Adjusted height for images */}
                                    <img
                                        src={product.image1}
                                        alt={product.name}
                                ...
                                <div className="py-4 px-2"> {/* Adjusted padding */}
                                    <div className="flex w-full justify-between">

                                        <h3 className={`font-medium font-extralight tracking-widest uppercase mb-1 line-clamp-2 ${textColor}`}>{product.bname}</h3>

                                        <h3 className={`font-medium font-bold tracking-widest uppercase mb-1 line-clamp-2 ${textColor}`}>{product.pname}</h3>
